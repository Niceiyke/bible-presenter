...
- Rust: `src-tauri/src/remote/` (`mod.rs`, `server.rs`, `auth.rs`, `sessions.rs`, `hub.rs`, `protocol.rs`, `commands.rs`, `snapshot.rs`, `assets.rs`). The server reuses the last bound local port (persisted in `remote_port.txt` under the app data dir, falling back to a random port if it is taken), keeps authoritative state in `state.remote` (`RemoteControl` in `mod.rs`), and never duplicates live/staged/settings state.
