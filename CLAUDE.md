...
- The Phase 5 source registry (`src/system/sourceRegistry.ts` + `src/hooks/useCameraSource.ts` + `src/hooks/useSharedLocalCameraStream.ts`): a per-device `SourceState` (`stream` + `status` ∈ idle/opening/connected/error/reconnecting/disconnected + `kind` local/phone/native/ndi + `errorKind`) unified across local webcams, phone cameras, and future capture devices. Local cameras are opened ONCE per device via `getUserMedia`, ref-counted across every consumer (`acquireSource`/`releaseSource`), and auto-reconnect once on device loss; phone sources are *registered* by the WebRTC host (`setPhoneSource`/`removePhoneSource` in `usePhoneCameraHost.tsx`, status from `phoneStatusFromConnection`) so signaling stays separate from acquisition. Synthetic `phone-camera-`/`native:`/`ndi:` ids are never sent to `getUserMedia` (`resolveSourceKind`). `useCameraSource` is the unified consumer hook; `useSourceStatus` is the non-acquiring picker variant; `useSharedLocalCameraStream(s)` remain back-compat wrappers. `ZoneCamera` in `CompositionRenderer` shares the registry stream (no per-zone `getUserMedia`) and `ZoneCamera`/`CameraFeed` render safe fallbacks. `SourcePicker` (`src/components/sources/SourcePicker.tsx`) lists local + phone sources with live status (used by the SceneBuilder camera source).
- The Phase 6 shared audio graph (`src/system/audioGraph.ts` + `src/hooks/useAudioGraphProvider.tsx`): one app-level program-audio bus. A single `getUserMedia` input track (processing off) is routed through a `GainNode` program bus and exposed as a shared post-gain `programTrack` that BOTH `RecordingProvider` (combined into the recorded stream) and `StreamingProvider` (`getSourceTracks().audio`, cloned per destination) draw from — so the app holds one audio capture, and volume/mute is one shared policy (mute never touches visual output). `AudioGraphProvider` is mounted in `App.tsx` and gates enabling on the Premium tier (`blocked`). State: `enabled`/`deviceId`/`devices`/`status` (idle/opening/connected/error/reconnecting/disconnected)/`error`/`programTrack`/`volume`/`muted`/`startedAt`. An independent monitor policy (`monitorTrack` = raw pre-gain input + `monitorVolume`/`monitorMuted`) keeps program mute/volume from ever affecting local operator monitoring and vice versa (WP8/P2-2). Device-loss auto-reconnects once; `retryAudio` re-opens. The RTMP AAC loopback (`rtmp_send_audio` in `useRtmpEncoder`) now encodes the shared graph track — no per-tab audio captures remain. A/V sync policy: program audio and the compositor video track are both real-time live captures, cloned together at start (per-sample packet timestamping is Phase 7).
- The Phase 7 shared encoder / packet bus (`src/system/programEncoder.ts`): ONE `VideoEncoder` per visual profile over the one compositor capture track, fanned to every RTMP/NDI transport. `startProgramEncoder(track, profile)` (called in `StreamingProvider.goLive`) is idempotent per profile; `stopProgramEncoder` closes it on Stop All. Transports `subscribeProgramPackets(cb)` and receive `EncodedPacket { sequence, timestampUs, keyframe, codec, width, height, fps, size, bytes }` (timestamps from the source `VideoFrame` timeline); consumers return `false` to signal backpressure and the module counts `packetsDropped`. `useRtmpEncoder`/`useNdiSender` subscribe when `programEncoderIsLive()` (forwarding to `rtmp_send`/`ndi_send`) and fall back to a per-session encoder otherwise — so N destinations never create N encoders, while standalone behavior and tests are preserved. `StreamerTab` surfaces encoder status (live/error + packets/dropped/consumers). Destination caps are enforced in `goLive` (defense in depth) beyond the Add-button gate.
...
- The Phase 6 RTMP ingest (`src/hooks/useRtmpEncoder.ts` + `src/components/StreamerTab.tsx`, backend `src-tauri/src/commands/rtmp.rs`) adds an RTMP mode: the compositor is encoded once with WebCodecs H.264 (Annex-B, `MediaStreamTrackProcessor` pulls `VideoFrame`s), packets are streamed to the backend, and long-lived `ffmpeg -f h264 -i pipe:` processes mux with `-c copy` (no re-encode) to the RTMP URLs. Backend commands `rtmp_start`/`rtmp_send`/`rtmp_stop`/`rtmp_status` (plus `rtmp_send_audio`) manage sessions keyed by `session_id` in a `HashMap` on `AppState.rtmp`, each with a writer thread draining an mpsc channel into ffmpeg stdin. `rtmp_start` takes the capture `fps` and feeds ffmpeg `-framerate`; `rtmp_send`/`rtmp_send_audio` return a typed `queued`/`dropped`/`closed` result and the backend tracks `sent`/`dropped`/`queued` counters surfaced by `rtmp_status` (WP3/P0-3, P1-1). The frontend `useRtmpEncoder`/`useStreamer` run a bounded reconnect (3 attempts, exponential backoff) for transient failures, reusing the destination id and never duplicating a session. Optional audio (Phase 6.1): an input device (mic / line-in / mixer feed, audio processing off) is captured via `getUserMedia`, encoded with WebCodecs `AudioEncoder` (AAC-LC), wrapped in ADTS headers, and streamed via `rtmp_send_audio` to a backend loopback TCP socket that ffmpeg reads as a second `-f adts -i tcp://127.0.0.1:<port>` input (muxed `-c:a copy`). ffmpeg resolves bundled-first via `src-tauri/src/binpaths.rs` (`{resource_dir}/bin/ffmpeg.exe` + `ffprobe.exe`, shipped as `bundle.resources`; fetch once with `scripts/fetch-ffmpeg.ps1` — LGPL build, not committed) with a PATH fallback, and errors cleanly when missing.
- The Phase 6.2 multi-platform Streaming Hub (`src/hooks/useStreamingProvider.tsx` + `src/components/StreamerTab.tsx` + `src/components/streaming/`): the streamer streams the single compositor feed to any number of destinations at once. The hub pipeline — compositor, destination set, shared audio, master transport, per-card handles — is owned by the app-scoped `StreamingProvider` (mounted in `App.tsx` beside `RecordingProvider`), so a broadcast survives navigating away from the workspace; `StreamerTab` is a thin view that previews the provider's live stream via a `<video>` element and drives its controls. Each destination is a platform preset (YouTube / Facebook Live / Twitch / Custom RTMP / Custom WHIP in `presets.ts` — the ingest server is pre-filled, the operator pastes only their stream key) rendered as a `DestinationCard` owning its transport (`useRtmpEncoder` with a `sessionId`, or `useStreamer`). The compositor video track and the shared audio input are **cloned per destination** (a track allows only one `MediaStreamTrackProcessor`). Master **Go Live All** / **Stop All** control every enabled destination; per-card status/bitrate/error are shown. Destinations persist as `stream_destinations` (`StreamDestination` in `outputs.rs` + TS) on the `stream-main` output config, seeded from the legacy `streaming` field when present.
