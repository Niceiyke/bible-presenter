...
- `capture`: transparent, always-on-bottom webview on the primary monitor (sized to fit, click-through, off the taskbar, never focused) that renders the same `OutputWindow` DOM surface; it is the **fallback** WGC source for the recorder/streamer, used only while the projection window is OFF so recording/streaming works even when the projector is closed. While the `output` window is on screen, a live session captures the REAL `output` window instead (one readback of the projected pixels), and the capture window stays hidden. It is **hidden by default** and only revealed while a session is sourcing it (`ensure_capture_visible` before WGC binds, `maybe_hide_capture` after the last such session stops; `swap_capture_source`/`sync_capture_sources` move a live session from the output window to the capture window — and back — on projector toggles via `set_output_visible`, swapping the writer's frame channel so ffmpeg never hits EOF) — during a switched-to session it MUST intersect the desktop, since a hidden/minimized window stops delivering new frames and capture freezes on the first idle frame, while WGC captures covered windows fine so the operator never sees it. Never revealed through `outputs_set_visible`.
...
- `commands/license.rs`: license status, activation, refresh, and deactivation. State lives in `src/license.rs` (`LicenseManager`, Arc-managed on `AppState.license`, persisted to `license.json` under the app data dir) and validation runs against a Cloudflare Worker in `workers/license/` (deploy instructions + admin API in its README; the endpoint host is set in `default_server_url()` and overridable via `WORDLYTE_LICENSE_URL`). Every Worker `/validate` response and every persisted record is **Ed25519-signed** (see "Signed license payloads" below); overriding `WORDLYTE_LICENSE_URL` with a stub cannot grant anything because a stub cannot produce a valid signature.
...
- **Machine fingerprint**: `machine_uid::get()` (Windows `MachineGuid`, `/etc/machine-id`, macOS UUID) hashed to a 64-hex id and sent to the license server (no PII). The persisted `license.json` record is bound to that id — copying the app data dir or install folder to another PC fails both the local machine check (`invalid`) and the record's Ed25519 signature, and re-activation on a new PC hits the server-side device-slot cap.
- **Signed payloads (anti-tamper / anti-stub)**: the license Worker signs every `/validate` response and the client stores exactly what was signed as a **v2 record**. The canonical serialization is `wl2|` + `[license_key, machine_id, status, issued_at, expires_at, tier, max_machines, machines_used, server_time|last_validated_at]` joined with `|` — it MUST match between the Worker (`signValidation` in `workers/license/src/index.js`) and the client (`canonical_license` / `verify_server_payload_with` in `src-tauri/src/license.rs`); reordering/renaming any field breaks every install. Verification uses the embedded public key (`SIGNING_PUBLIC_KEY_B64` in `license.rs`; regenerate the keypair with `workers/license/scripts/generate-keypair.mjs` and deploy the private half as the Worker secret `LICENSE_SIGN_PRIVATE_KEY` — the private key must NEVER be committed, it lives in `workers/license/.license-sign-private.jwk` and `.dev.vars`). The record is stored **DPAPI-encrypted** (current-user, `CRYPTPROTECT_UI_FORBIDDEN`) on Windows, so a hand-edited or copied `license.json` fails signature verification and the app **fails closed to `Invalid`** (repairable only by an online revalidation; `refresh` migrates pre-signing v1 records in the same pass). `last_seen_at` is deliberately unsigned (it only drives the clock-rollback tripwire). A background `license::spawn_periodic_refresh` (invoked from `main.rs` setup) re-validates ~60s after startup then every ~6h with jitter, publishing `license-updated` so revocations propagate without the operator opening Settings.
