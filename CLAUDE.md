...
- `capture`: transparent, always-on-bottom webview on the primary monitor (sized to fit, click-through, off the taskbar, never focused) that renders the same `OutputWindow` DOM surface; it is the dedicated WGC source for the recorder/streamer so recording/streaming works even when the projection window is closed. It MUST intersect the desktop — a fully off-screen/hidden/minimized window stops delivering new frames and capture freezes on the first idle frame — while WGC captures covered windows fine, so the operator never sees it in practice. Never revealed through `outputs_set_visible`.
