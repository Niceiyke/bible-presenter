...
- The Phase 6 RTMP ingest (`src/hooks/useRtmpEncoder.ts` + `src/components/StreamerTab.tsx`, backend `src-tauri/src/commands/rtmp.rs`) adds an RTMP mode: the compositor is encoded once with WebCodecs H.264 (Annex-B, `MediaStreamTrackProcessor` pulls `VideoFrame`s), packets are streamed to the backend, and a long-lived `ffmpeg -f h264 -i pipe:` muxes with `-c copy` (no re-encode) to the RTMP URL. Backend commands `rtmp_start`/`rtmp_send`/`rtmp_stop`/`rtmp_status` manage the session on `AppState.rtmp` with a writer thread draining an mpsc channel into ffmpeg stdin. Optional audio (Phase 6.1): the operator enables an input device (mic / line-in / mixer feed, audio processing off) in `StreamerTab`; it is captured via `getUserMedia`, encoded with WebCodecs `AudioEncoder` (AAC-LC), wrapped in ADTS headers, and streamed via `rtmp_send_audio` to a backend loopback TCP socket that ffmpeg reads as a second `-f adts -i tcp://127.0.0.1:<port>` input (muxed `-c:a copy`). ffmpeg is resolved from PATH and errors cleanly when missing; the `stream-main` `streaming` field persists `mode: "whip" | "rtmp"` + url + stream key.
