...
Sidecar bundling: `wordlyte-engine.exe` ships as a bundle resource (`binaries/wordlyte-engine.exe` → resource dir, resolved by `main.rs` via `resource_dir()`). Because the engine is a `[[bin]]` of the same crate, tauri-build's build script would fail a fresh checkout (the resource does not exist until cargo build produces it). `scripts/copy-engine-binary.mjs` breaks the loop in two modes: `--bootstrap` (wired into `beforeBuildCommand`/`beforeDevCommand`) writes a 0-byte placeholder before cargo build, and the no-arg mode (`beforeBundleCommand`) overwrites it with the freshly built binary before bundling — failing loudly if the binary is missing. The CI Check workflow touches the same placeholder for bare `cargo check/clippy/test`. The engine links libav dynamically through `FFMPEG_DIR`: run `scripts/fetch-ffmpeg.ps1` once to stage the pinned BtbN SHARED GPL build into `src-tauri/ffmpeg-dist/` (include/lib/bin — point `FFMPEG_DIR` here; no vcpkg or pkg-config needed) plus the runtime DLLs into `src-tauri/binaries/dll/`. Those DLLs (`av*.dll`/`sw*.dll`, names embed the libav major version) ship at the **bundle root beside both exes** via the `binaries/dll/*.dll` → `.` resource mapping: they are load-time static imports, and the Windows loader searches only the exe's own folder (+ system dirs/PATH) before any app code runs — a `bin/` subfolder plus the runtime `SetDllDirectoryW` in `main.rs` can never satisfy them (this caused "avcodec-63.dll was not found" popups in 2.0.2). `src-tauri/build.rs` stages the same DLLs from vcpkg / `$FFMPEG_DIR/bin` (or pre-staged `binaries/dll`) into `target/<profile>` so `cargo run` / dev builds launch. CI caches the fetched artifacts keyed on the script hash; both workflows dropped their vcpkg/pkg-config steps for this.
...
- `engine/capture.rs` (Phase I1): Media Foundation camera enumeration (`list_video_devices()` → friendly names via `MFEnumDeviceSources`; non-Windows stub errors). Live capture itself is dshow ffmpeg in `media.rs`: `camera_capture_command` (720p30 default profile — `CameraBackground` carries no quality fields), `camera_spawner(ffmpeg)`, and hub keys `cam:{device}@{w}x{h}@{fps}` (parsed from the RIGHT so device names may contain `@`). `collect_video_refs` emits cam: keys for Camera items/backgrounds/zones; the renderer draws them through the same video arms (contain item, zone rect+opacity, cover background with optional mirror). `MediaFrameHub.pin/unpin(key)` pre-warms a capture with no program reference (`capture_start`/`capture_stop`); `capture_status()` reports live keys; `cam:`-prefixed Ended|Failed drain as `EngineEvent::CaptureDeviceLost`. Hub lock order is ALWAYS `pinned` → `entries`. Failure policy: `capture_start` validates the friendly name against MF before pinning ("camera not found"), and decoder respawn backs off exponentially per consecutive failure (`retry_after × 2^n`, 60s cap) so a missing device can't spam ffmpeg. The webview/engine ID bridge lives in `src/system/cameraNames.ts`: broadcast items are staged with the ENGINE friendly name (matched by label — Chromium labels come from the same MF property), previews translate back via `webviewDeviceIdForEngineName` inside `sourceRegistry.openLocal`, hydrated from `refreshCameras`. Phone WebRTC and NDI sources join the same hub later per the Phase I taxonomy in `docs/RUST_VIDEO_ENGINE_PLAN.md`.
